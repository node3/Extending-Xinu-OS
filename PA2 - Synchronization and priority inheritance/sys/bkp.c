...
void print_proctab(int mypid, int lck){
	kprintf("\n %d,  status: %d, type: %d, priority: %d, blocking_time: %d\n", mypid, proctab[mypid].plocks[lck].status, proctab[mypid].plocks[lck].type, proctab[mypid].plocks[lck].priority,proctab[mypid].plocks[lck].blocking_time);

}


void print_loctab(int lck){
	kprintf("\n %d,  lstate: %d, lacq: %d, lcnt: %d, lqhead: %d, lqtail: %d\n", lck, locks[lck].lstate, locks[lck].lacq, locks[lck].lcnt, locks[lck].lqhead, locks[lck].lqtail);
}
...
void test2 ()
{
	...
	rd3 = create(reader2, 2000, 20, "reader2", 3, 'D', lck, 10);
	rd4 = create(reader2, 2000, 20, "reader2", 3, 'E', lck, 10);
        wr1 = create(writer2, 2000, 20, "writer2", 3, 'C', lck, 20);
	
        wr2 = create(writer2, 2000, 20, "writer2", 3, 'F', lck, 20);

	kprintf("-start writwer 2, then sleep 1s. lock granted to reader A\n");
        resume(wr2);
        //ppprocs(wr2);
        sleep (5);
...
void test3 ()
{
        ...
        resume(rd1);
        sleep (1);
	assert (getprio(wr1) == 25, "1 Test 3 failed, getprio(wr1) == 25");

        kprintf("-start reader B, then sleep 1s. reader B(prio 30) blocked on the lock\n");
        resume (rd2);
	sleep (1);
	assert (getprio(wr1) == 30, "2 Test 3 failed, getprio(wr1) == 30");
	
	kprintf("-kill reader B, then sleep 1s\n");
	kill (rd2);
	sleep (1);
	assert (getprio(wr1) == 25, "3 Test 3 failed, getprio(wr1) == 25");

	kprintf("-kill reader A, then sleep 1s\n");
	kill (rd1);
	sleep(1);
	assert(getprio(wr1) == 20, "4 Test 3 failed, getprio(wr1) == 20");
