					...
					/* array in the code)		*/
{
	...
	*saddr = MAGIC;
	...
	for (i=0; i< NLOCK; i++){
		pptr->plocks[i].status = L_UNDEFINED;
		pptr->plocks[i].priority = 0;
	}
	...
	// LOCK arguments
	if (nargs >= 2) {
		pptr->plocks[*(&args+1)].status = L_CREATED;
		if (nargs > 2) {
			pptr->plocks[*(&args+1)].priority = *(&args+2);
		}
	}
	...
	for ( ; nargs > 0 ; nargs--)	/* machine dependent; copy args	*/
		//pptr->plocks[*a].status = L_CREATED;
