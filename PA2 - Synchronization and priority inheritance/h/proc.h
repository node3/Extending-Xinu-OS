...
#ifndef	NLOCK				/* set the number of locks	*/
#define	NLOCK		50		/*  allowed if not already done	*/
#endif
...
struct	plock	{
	int status;	// L_CREATED, L_UNDEFINED or L_DELETED
	int type;	// READ or WRITE
	int waiting;	// TRUE or FALSE
	int priority;
...
struct	pentry	{
	...
	int	pwaitret;
	int	pinh;
	struct	plock	plocks[NLOCK];		//locks to be held
};
