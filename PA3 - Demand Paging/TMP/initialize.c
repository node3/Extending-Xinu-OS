...
*/
	...
	//kprintf("\nFrame table initialisation");
	init_frm();
	//kprintf("\nBS Map initialisation");
	init_bsm();
	//kprintf("\nSet evec for pfintr");
	set_evec(14, pfintr);
	//kprintf("\nInitialise global page table");
	init_gpt();
	//kprintf("\nInitialise replacement policy tables");
	init_policy();
	//kprintf("\nGet page directory for NULLPROC");
	get_pd(NULLPROC);
	//kprintf("\nWrite NULLPROC page dir [%ld] address to CR3", proctab[0].pdbr);
	write_cr3(proctab[0].pdbr);
	//kprintf("\nEnable demand paging [CR3: %ld]", read_cr3());
	enable_paging();
	//kprintf("\nDemand paging initialisation done");
